...
#        pdb.set_trace()
    ...
    def valid_v(self, lv, N, RSS=True):
        ...
        sT = np.ones((4, int(len(lv) / pndim)), dtype='bool')
        sTAB = np.ones(int(len(lv) / pndim), dtype='bool')
        ...
        if self.ndim == 3:
            B = (sDDB[0] * sDDB[1]).reshape(int(len(lv) / 8), 8)
            sB = np.sum(B, axis=1)
            ...
            return (EB, AB)
        if self.ndim == 2:
            B = (sDDB[0] * sDDB[1]).reshape(int(len(lv) / 4), 4)
            sB = np.sum(B, axis=1)
...
##########################################
        ...
        if clust != []:
            ...
            for i in range(len(clust)):
                if len(clust) < 3:
                    clusters = np.array(clust[i],dtype=int)
                else:

                    if len(np.shape(count)) > 1:
                        clusters = np.array(np.intersect1d(clust[count[i,0]], clust[count[i, 1]]),dtype=int)
                    else:
                        clusters = np.array(np.intersect1d(clust[count[0]], clust[count[1]]),dtype=int)

                clust_vol = np.sum(np.array(self.dlayer[l][dlindx].vol)[np.array(np.unique(clusters),dtype=int)])
